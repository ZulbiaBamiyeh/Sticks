...
- **Spiders:** hang on a thread and drop; eight IK legs that plant their steps.
- **Jellyfish:** a bell with tentacles that trail as it swims in pulses.
- **Brute:** a solid knuckle-walking silhouette; a slam shockwave and a lobbed glob.

What makes them look finished: solid bodies with smooth silhouettes, small
light highlights for eyes and shine, and at most one accent colour. Two early
tries (a scribble ball and a crumpled-paper golem) read as goofy and were cut.
Elites are marked by red eyes. The plan for growing the roster is in
`docs/bestiary.md`.
...
2. Six hunts at three tiers each: Slime (the Elite is a Slime King that splits
   twice), Bats (the Elite swarm dives two at a time), Serpent, Spiders,
   Jellyfish (poison stings) and Brute (slams twice past Easy). Fighters pick
   the nearest creature as their target. Each creature is one entry in `MOB`
   with its own think, attack, step and draw.
