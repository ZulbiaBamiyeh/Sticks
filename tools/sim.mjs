...
//   2. random loadouts on shared layouts: win rate per weapon, per skill, per acrobatics level, and weapon against weapon.
...
const out = await p.evaluate(([FIGHTS, SEED]) => {
  ...
  // 1. the ladder
  const me = { weapon: 'sword', skill: 'blink', acro: 2, pieces: T[0] }, N = 8;
  const ladder = H.rivals.map((rv, i) => {
    let w = 0, l = 0, secs = 0;
    for (let s = 0; s < N; s++) {
      const r = H.fight(100 * i + s + 1, me, { name: rv.name, weapon: rv.weapon, skill: rv.skill, acro: rv.acro, pieces: T[rv.t] });
      if (r.winner === 0) w++; else if (r.winner === 1) l++;
      secs += r.t / 60;
    }
    return { rival: `${rv.name} (${rv.weapon} · ${rv.skill} · acro ${rv.acro})`, 'you win': pct(w, N), 'rival wins': pct(l, N), 'avg secs': (secs / N).toFixed(1) };
  });
  ...
  return { invalid, ladder, weapons: rate(ws), skills: rate(ks), acro: rate(as), mat, perFight, draws, avg: (secs / FIGHTS).toFixed(1) };
}, [FIGHTS, SEED]);

console.log(out.invalid.length ? out.invalid.join('\n') : 'Every layout validates with every weapon.');
console.log('\nThe ladder: sword + blink + acrobatics 2 on layout 0 against each rival');
